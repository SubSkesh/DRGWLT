...
class _WalletDetailScreenState extends State<WalletDetailScreen> {
  ...
  void _onAddPressed(TxType? dealType) {
    if (dealType != null) {
      context.router.push(
        AddDealRoute(
          dealType: dealType,
          preSelectedWalletId: widget.walletId, // ← Passa l'ID del wallet corrente
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_wallet!.name,style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold,fontFamily: 'Game'),),
      ),
      body: Padding(
          ...
          children: [
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    _buildStatRow('Total Deals', _wallet!.totalDeals?.toString() ?? '0'),
                    _buildStatRow('Total Spent', '€${_wallet!.totalSpent?.toStringAsFixed(2) ?? "0.00"}'),
                    _buildStatRow('Totale Guadagnato', '€${_wallet!.totalEarned?.toStringAsFixed(2) ?? "0.00"}'),
                    _buildStatRow('Profitto Netto', '€${_wallet!.netProfit?.toStringAsFixed(2) ?? "0.00"}'),
                  ],
                ),
              ),
            ),
            ...
            Expanded(
              child: _deals.isEmpty
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    title: Text(deal.type.toString().split('.').last),
                    subtitle: Text('${deal.amount} x €${deal.pricePerUnit.toStringAsFixed(2)}'),
